...
namespace WindowsFormsApp1
{
    public partial class Form222 : Form
    {
        ...
        void BorderTraverse4(Bitmap bmp, int x, int y, Color fill, Color old)
        {
            ...
            else { bmp.SetPixel(x, y, fill); }
            pictureBox1.Image = bmp;
        }

        void RestoreColors(Bitmap bmp, Bitmap tempbmp, Color tempColor) {
            for (int x = 0; x < bmp.Width; x++)
            {
                for (int y = 0; y < bmp.Height; y++)
                {
                    if (bmp.GetPixel(x, y) != tempbmp.GetPixel(x, y) && tempbmp.GetPixel(x, y) == tempColor) 
                    {
        ...
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (radioButton5.Checked) {
                if (e.Button == MouseButtons.Left) 
                {
                    if (canvas.GetPixel(e.X, e.Y) == b)
                    {
                        MessageBox.Show("Please choose the different color.");
                    }
                    else
                    {
                        Bitmap tcanvas = new Bitmap(pictureBox1.Image);
                        Color tcolor = canvas.GetPixel(e.X, e.Y);
                        BorderTraverse4(canvas, e.X, e.Y, b, canvas.GetPixel(e.X, e.Y));
                        RestoreColors(canvas, tcanvas, tcolor);
                    }
                }
            }
